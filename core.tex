...
matching. we typecheck against the concrete set of effects and the value
signature. open sets have allow a implicit forwarding clause. it is overridden
if the user provides their own forwarding clause. closed effect sets must be
fully specified. special wildcard pattern for computations desugars to perform
forwarding of unhandled commands. this transformation results in additional
rows for the clause matrix and also requires a transformation of the
expression on the right hand side. it must execute the command and bind the
result to be used in a recursive call to the forwarding handler.
