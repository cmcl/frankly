...
\[
\begin{array}{@{}r@{~}c@{~}l@{}}
u, v &::=& x \mid f \mid k~\many{v} \mid \thunk{d}  \\ % values
d, e &::=& \lambda x.e \mid d~v                        % computations
    \mid c~\many{v} \mid \key{ret}~v \mid \force{u} \\
  &\mid& \key{case}~u~\key{of}~
...
\[
\begin{array}{@{}r@{~}c@{~}l@{}}
u, v &::=& x \mid k~\many{v} \mid \thunk{d}  % values
...
\key{handle}~E[c_j~\many{v}]~\key{with}
   ...
   x \mapsto e
  &\reducesto&
     e_j[\many{v}/\many{x_i}, \thunk{\lambda x.E[x]} / g] \\
...
\key{handle}~E[c~\many{v}]~\key{with}
   ...
   x \mapsto e
  &\reducesto&
     \key{let}~x=c~\many{v}~\key{in}~
       \key{handle}~E[x]~\key{with}
          (\handle{c_i~\many{x_i}}{g} \mapsto e_i)_i \medvert
