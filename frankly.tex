...
\item A series of type preserving translations from Frank into
  ...
  polymorphic variant of the \lameff calculus of Kammar et
  al~\cite{KammarLO13}, which in turn is an extension of Levy's
...
%
The differences between Frank and Core Frank are all in the checkable
computation terms.

NONSENSE. Need to restore the full syntax and type rules for Core
Frank.
%
The syntax for Core Frank checkable computations is as follows.
\begin{syntax}
&e  &::= & v \mid \lambda x.e \\
...
\end{syntax}%
%
The Core Frank typing rules for checkable computations are given in
Figure~\ref{fig:core-typing}.
...
\caption{Core Frank Typing Rules for Checkable Computation Terms}
...
Given a Frank expression $\thunk{e}$ such that $\idoesg{R_1 \dots R_n
  \to R}{e}$ we compile it to an equivalent Core Frank expression.
%
...
In our scenario, pattern matching trees $M$ are build up from leaves,
case analysis, and handlers.
%% \begin{equations}
%% M &::= & \many{e} \\
...
%
Our default strategy is to class incomplete pattern matching as a type
error, and take the first element in the case of ambiguous pattern
matching.
%
...
Of course, because Core Frank now takes values as arguments whereas
Frank takes computations, each argument must be wrapped in a thunk
constructor.

The type translation is straightforward.
