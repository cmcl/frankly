...
\newcommand{\pat}[3]{{#1} \judgeword{lets} {#2} \dashv #3}
...
C    &::= \rt{V} \mid \effbox{\sigs}V \to C \\
\\
\sig  &::= \cdot \mid c : U(\many{V}), \sig \\
\sigs &::=
  \emptyset \mid \varepsilon \mid \sigs, \sig~\many{V} \\
\\
\Gamma &::= \cdot \mid \Gamma, x:V \mid f:\forall \varepsilon \many{X}.V \mid g:C \\
\end{array}
\]
...
v &::=& u \mid k~\many{v}                               % has
          \mid \thunk{e}
\\[1ex]
d &::=& g \mid c~\many{v} \mid d~e \mid \force{u}    \\ % can
e &::=& v \mid r \mapsto e \mid e \medvert e            % does
\\[1ex]
...
\inferrule
  {\sigs' \text{ closed} \\
   \pat{\effbox{\sigs'}V}{r}{\Delta} \\
   \does{\Gamma, \Delta}{\sigs}{C}{e}}
  {\doesgs{\effbox{\sigs'}V \to C}{r \mapsto e}}
...
\inferrule
  ...
  {\pat{\effbox{\sigs}V}
       {\handle{c~\many{p}}{g}}
       {\many{\Delta}, g:{\effbox{}U \to \effbox{\sigs}V}}}
...
%%           \mid \thunk{\many{m} \to t}  \\ 
%%   &\mid& \key{case}~x~\key{of}~
%%            \many{(k~\many{x} \mapsto t)} \\
%%   &\mid& \key{handle} ~m~ \key{with}~
...
v &::=& u \mid k~\many{v}                            % has
          \mid \thunk{e}  \\[1ex]
d &::=& c~\many{v} \mid d~v \mid \force{u} \\        % can
...
%%    \mid \lambda g.e \mid d~e \mid \key{ret}~v \mid \force{u} \\
%%   &\mid& \key{case}~u~\key{of}~
...
\[
\begin{array}{@{}r@{~}c@{~}l@{}}
u, v &::=& x \mid f \mid k~\many{s} \mid \thunk{d}  \\ % values
