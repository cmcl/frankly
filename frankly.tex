...
abort = bind (aborting!) ()
\end{verbatim}
The term \verb!()! denotes an empty set of pattern matching clauses
covering the \verb!Zero! return type of \verb!aborting!.
...
\end{verbatim}
%
The computation \verb$receive!$ is the command that delivers a list by
receive; the value \verb$receive$ is the name of the command which
receives.
...
as arguments, hence we call them \emph{multi-handlers}. In fact
standard functions like the ones we have seen so far are just special
cases of multi-handlers in which the arguments all have the empty
effect.
...
\verb!send!  commands from one computation by matching them with
...
\end{verbatim}
%
The type signature says the following. It must handle all
\verb!Send X! commands in its first argument and all \verb!Receive X!
  commands in its second argument. The first argument returns values
  of type \verb!Unit!; the second argument returns values of type
  \verb!Y!. The handler itself is allowed to perform \verb!Abort!
...
Here is a main function, which plugs \verb!sender! and \verb!catter!
together and sends their output to the console.
%
\begin{verbatim}
...
\slab{checkable computations} &e &::=& v \mid \many{r} \mapsto e \mid e \medvert e
\\[1ex]
...
The translation on types
  ...
  %% lift({C}) = [ lift(effs(C)) ]{ lift(C) }

  %% lift([Sig]V -> C) = [ lift(Sig) ]lift(V) -> lift(C)
  %% lift([Sig]V) = < lift(V) >
