...
t &::=& s \mid k~\many{t}                            % has
  ...
  &\mid& \keyword{case}~x~\keyword{of}~
           \many{(k~\many{x} \mapsto t)} \\
  &\mid& \keyword{handle} ~s~ \keyword{with}~
           \many{(\handle{c~\many{x}}{g} \mapsto t)} \medvert
            x        \mapsto t \\
...
\inferrule
  {\sigs' \text{ closed} \\
   \has{\Gamma}{\sigs~\sigs'}{V}{s} \\
   (\has{\Gamma, \many{x_c}:\many{U}, g:\thunk{U' \to \effbox{\sigs~\sigs'}V}}
       {\sigs}{W}{t_c})_{c : U'(\many{U}) \in \sigs'} \\
   \has{\Gamma, x:V}{\sigs}{V'}{t}}
  {\has{\Gamma}{\sigs}
     {V'}{\keyword{handle}~ s ~\keyword{with}~
           \many{\handle{c~\many{x_c}}{g} \mapsto t_c} \medvert
           x \mapsto t}}
\end{mathpar}
...
\inferrule
  {\hasgs{V}{t}}
  {\chasg{\effbox{\sigs}V}{t}}

\inferrule
  {\chas{\Gamma, x:V}{C}{h}}
  {\chasg{V \to C}{\lambda x.h}}
\end{mathpar}
