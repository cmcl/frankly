...
\begin{array}{@{}r@{~}c@{~}l@{}}
s &::=& x \mid f                                  \\ % makes
f &::=& c~\many{t} \mid f~t \mid \force{s} \\ % can
t &::=& s \mid k~\many{t}                            % has
          \mid \thunk{h}  \\ 
h &::=& t \mid \lambda x.h \\                        % does
  &\mid& \keyword{case}~s~\keyword{of}~
           \many{(k~\many{x} \mapsto h)} \\
  &\mid& \keyword{handle} ~f~ \keyword{with}~
           \many{(\handle{c~\many{x}}{g} \mapsto h)} \medvert
            x        \mapsto h \\
\end{array}
\]
...
\inferrule
  {\does{\Gamma}{\sigs'}{C}{h}}
  {\hasgs{\effbox{\sigs'}\thunk{C}}{\thunk{h}}}
\end{mathpar}
...
\inferrule
  {\does{\Gamma, x:V}{\sigs}{C}{h}}
  {\doesgs{V \to C}{\lambda x.h}}

\inferrule
  {\makesgs{s}{D~\many{U}} \\
   (\does{\Gamma, \many{x_k}:\many{V}}{\sigs}{C}{h_k})_{k~\many{V} \in D~\many{U}}}
  {\doesgs{C}{\keyword{case}~ s ~\keyword{of}~
               \many{(k~\many{x_k} \mapsto h_k)}}}

\inferrule
  {\sigs' \text{ closed} \\
   \can{\Gamma}{\sigs~\sigs'}{f}{F~V} \\
   (\does{\Gamma, \many{x_c}:\many{U}, g:\effbox{\sigs~\sigs'}\thunk{U' \to F~V}}
         {\sigs}{C}{h_c})_{c : U'(\many{U}) \in \sigs'} \\
   \does{\Gamma, x:V}{\sigs}{C}{h}}
  {\does{\Gamma}{\sigs}
         {C}{\keyword{handle}~ f ~\keyword{with}~
               \many{\handle{c~\many{x_c}}{g} \mapsto h_c} \medvert
                x \mapsto h}}
\end{mathpar}
