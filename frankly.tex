...
C    &::= \effbox{\sigs}V \mid \many{\effbox{\sigs}V} \to C \\
\\
...
\inferrule
  {\cangs{d}{\many{\effbox{\sigs'}V} \to C} \\
   (\idoesg{\effbox{\sigs~\sigs'_i}V_i}{e_i})_i}
  {\can{\Gamma}{\sigs}{d~\many{e}}{C}}

\inferrule
  {\makesgs{u}{\thunk{C}} \\ \mathit{effs}(C) = \sigs}
  {\cangs{\force{u}}{C}}
...
\end{equations}%
where for $H = (\handle{c_i~\many{x_i}}{g} \mapsto e_i)_i \medvert x
\mapsto e$:
\begin{equations}%
H(\key{ret}, v) &=& e[v/x] \\
