...
M = \bstack
    ...
    \quad\quad\quad \handle{\var{receive}}{r} \mapsto e_1 \\
    \quad\quad\quad z \mapsto e_2 \\
    \quad  \handle{abort}{s} \mapsto \\
    \quad\quad \key{handle}~\force{x_1}~\key{with} \\
    \quad\quad\quad    \handle{receive}{r} \mapsto \var{bind}~(\force{r}~\force{receive})~e_3 \\
    \quad\quad\quad    z \mapsto e_3~z \\
    %% \quad\quad\quad    z \mapsto \var{bind}~\force{z}~e_3 \\
    \quad  y \mapsto \\
    ...
    \quad\quad\quad\quad    z \mapsto e_4~z \\
    %% \quad\quad\quad\quad    z \mapsto \var{bind}~\force{z}~e_4 \\
\estack
\]
where $\var{bind}$ is defined in Section~\ref{sec:examples}. We obtain
the corresponding Core Frank code by abstracting over the fresh
variables.
%
\[
\lambda x_0~x_1.M
